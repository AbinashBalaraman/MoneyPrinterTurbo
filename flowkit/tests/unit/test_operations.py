...
class TestGenerateSceneImage:
    ...
    @pytest.mark.asyncio
    async def test_logs_error_when_named_entity_is_not_linked(
        self, service, base_scene, mock_client
    ):
        """A declared name matching no linked entity must not be dropped silently.

        The still would otherwise render un-conditioned — the character comes
        back looking like someone else, with nothing anywhere reporting it.
        """
        with patch("agent.sdk.services.operations.crud") as mock_crud, \
             patch("agent.sdk.services.operations.logger") as mock_logger:
            mock_crud.get_project = AsyncMock(return_value={"user_paygate_tier": "PAYGATE_TIER_TWO"})
            mock_crud.get_project_characters = AsyncMock(return_value=[])

            await service.generate_scene_image(base_scene, "VERTICAL")

        assert mock_client.generate_images.call_args.kwargs["character_media_ids"] is None
        mock_logger.error.assert_called_once()
        # args: (format, scene_id, sorted(unknown), pid, pid)
        assert set(mock_logger.error.call_args[0][2]) == {"Hero", "Castle"}

    @pytest.mark.asyncio
    async def test_no_error_when_all_named_entities_are_linked(
        self, service, base_scene, mock_client
    ):
        project_chars = [
            {"name": "Hero", "slug": "hero", "media_id": SAMPLE_UUID},
            {"name": "Castle", "slug": "castle", "media_id": SAMPLE_UUID_2},
        ]
        with patch("agent.sdk.services.operations.crud") as mock_crud, \
             patch("agent.sdk.services.operations.logger") as mock_logger:
            mock_crud.get_project = AsyncMock(return_value={"user_paygate_tier": "PAYGATE_TIER_TWO"})
    ...
    @pytest.mark.asyncio
    async def test_reports_only_the_unmatched_names(self, service, base_scene, mock_client):
        project_chars = [{"name": "Hero", "slug": "hero", "media_id": SAMPLE_UUID}]
        with patch("agent.sdk.services.operations.crud") as mock_crud, \
             patch("agent.sdk.services.operations.logger") as mock_logger:
            mock_crud.get_project = AsyncMock(return_value={"user_paygate_tier": "PAYGATE_TIER_TWO"})
            mock_crud.get_project_characters = AsyncMock(return_value=project_chars)
        ...
        assert list(mock_logger.error.call_args[0][2]) == ["Castle"]
        # Still uses the reference it could resolve rather than dropping all of them.
        assert mock_client.generate_images.call_args.kwargs["character_media_ids"] == [SAMPLE_UUID]
