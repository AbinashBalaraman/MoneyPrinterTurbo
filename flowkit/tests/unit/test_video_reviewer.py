        ...
        try:
            ...
            for sheet_idx, start in enumerate(range(0, 20, per_sheet)):
                ...
                for link, expected_target in zip(symlinks, expected_chunk):
                    # Symlink on POSIX, plain copy on Windows — compare bytes
                    # either way so the test holds on both.
                    assert link.read_bytes() == expected_target.read_bytes(), (
                        f"chunk {sheet_idx} file {link.name} content differs from "
                        f"{expected_target.name}"
                    )
        finally:
