...
from agent.utils.entity_match import declared_names, entity_names, matches_entity, resolve_matched
...
class OperationService:
    ...
    async def generate_scene_image(self, scene: dict, orientation: str) -> dict:
        ...
        char_media_ids = None
        char_names_set = declared_names(scene)
        if char_names_set and pid:
            project_chars = await crud.get_project_characters(pid)
            valid_ids = []
            missing_refs = []
            matched_names: set = set()
            for c in project_chars:
                if not matches_entity(c, char_names_set):
                    continue
                matched_names.update(entity_names(c))
                mid = c.get("media_id")
                if mid:
            ...
            # A declared name that matches no linked entity used to be dropped
            # silently: the still renders un-conditioned, so the character comes
            # back looking like someone else, and nothing anywhere reports it.
            # Make it loud rather than invisible. (Warn, not fail — the scene
            # still generates; consistency.py reports it afterwards.)
            unknown = char_names_set - matched_names
            if unknown:
                logger.error(
                    "Scene %s names %s but no entity linked to project %s "
                    "matches — generating WITHOUT reference conditioning, "
                    "so the character will not be consistent. Link them via "
                    "POST /api/projects/%s/characters/<id>.",
                    scene.get("id", "?")[:8], sorted(unknown), pid[:8], pid[:8],
                )
    ...
    async def edit_scene_image(self, scene: dict, orientation: str,
        ...
        if char_names_raw and pid:
            ...
            if isinstance(char_names_raw, list) and char_names_raw:
                ...
                for c in project_chars:
                    if matches_entity(c, char_names_set) and c.get("media_id"):
...
async def _build_video_prompt(base_prompt: str, scene: dict, project_id: str | None) -> str:
    ...
    if project_id and has_dialogue:
        ...
        if isinstance(char_names_raw, list) and char_names_raw:
            ...
            voices = []
            for c in project_chars:
                if matches_entity(c, char_names_set) and c.get("voice_description"):
