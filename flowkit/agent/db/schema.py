...
CREATE TABLE IF NOT EXISTS scene (
    ...
    conditioned_with TEXT, -- JSON array of names the image was ACTUALLY conditioned on;
                           -- NULL = unknown (pre-dates the column), '[]' = generated
                           -- without conditioning. Set at generation time; see the
                           -- migration note in init_db for why it is not backfilled.
...
-- Publishing is performed by shorts_content_engine, which reports each outcome
-- here so the dashboard can show it without a second backend.
--
-- ``is_mock`` is a column rather than a key buried in ``metadata`` on purpose.
-- The original publishing bug was a simulated upload being recorded as a real
-- one; the CHECK constraint below makes that state unrepresentable, so the
-- guarantee holds even if a future caller forgets to set the status.
CREATE TABLE IF NOT EXISTS publication (
    id            TEXT PRIMARY KEY,
    video_id      TEXT NOT NULL REFERENCES video(id) ON DELETE CASCADE,
    project_id    TEXT REFERENCES project(id) ON DELETE CASCADE,
    platform      TEXT NOT NULL,
    -- requested: a retry was asked for, nothing attempted yet
    -- published: a real platform accepted the upload
    -- dry_run:   simulated; nothing was posted
    -- failed:    attempted and rejected
    status        TEXT NOT NULL DEFAULT 'requested'
                  CHECK(status IN ('requested','published','dry_run','failed')),
    post_id       TEXT,
    video_url     TEXT,
    error_message TEXT,
    ...
    created_at    TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    updated_at    TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ', 'now')),
    -- A simulated upload may never carry a URL or claim to be published.
    CHECK (is_mock = 0 OR (status = 'dry_run' AND video_url IS NULL AND post_id IS NULL))
);
CREATE INDEX IF NOT EXISTS idx_publication_video ON publication(video_id);
...
""")
        ...
        if "conditioned_with" not in scene_columns:
            # Records which reference entities the generated image was actually
            # conditioned on. Three states, and the difference matters:
            #   NULL  -> unknown; the image predates this column
            #   '[]'  -> generated with NO conditioning (character inconsistent)
            #   '["arthur","rusty"]' -> conditioned on exactly these
            # Without it, "was this still conditioned?" can only be inferred from
            # the *current* project links, which answers a different question —
            # linking a character afterwards would make an un-conditioned image
            # look fine. Deliberately no backfill: guessing would defeat the
            # point, and NULL honestly means "we cannot tell".
            await db.execute("ALTER TABLE scene ADD COLUMN conditioned_with TEXT")
            logger.info("Migrated: added conditioned_with column to scene table")
