...
_VALID_TABLES = frozenset({"character", "project", "video", "scene", "request", "material", "publication"})
...
_COLUMNS = {
    ...
    "publication": {"status", "post_id", "video_url", "error_message", "is_mock", "metadata", "published_at", "updated_at"},
}
...
# ─── Publication ─────────────────────────────────────────────
#
# Publishing happens in shorts_content_engine; these rows are its report back.
# The honesty rule is enforced here as well as in the table's CHECK constraint
# so a bad caller gets a readable ValueError instead of sqlite3.IntegrityError.

async def record_publication(video_id: str, platform: str, status: str = "published",
                             post_id: str = None, video_url: str = None,
                             error_message: str = None, is_mock: bool = False,
                             metadata: dict = None, published_at: str = None,
                             project_id: str = None) -> dict:
    """Store one platform outcome for a video."""
    platform = (platform or "").strip().lower()
    if not platform:
        raise ValueError("platform is required")

    if is_mock:
        if status != "dry_run" or video_url or post_id:
            raise ValueError(
                "a simulated upload must be recorded as status='dry_run' with no url or post_id"
            )
    if status == "published" and not post_id:
    ...
    async with _db_lock:
        await db.execute(
            """INSERT INTO publication
               (id,video_id,project_id,platform,status,post_id,video_url,error_message,
                is_mock,metadata,published_at,created_at,updated_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (pid, video_id, project_id, platform, status, post_id, video_url,
             error_message, 1 if is_mock else 0,
             json.dumps(metadata or {}), published_at or now, now, now))
        await db.commit()
    return await _get_with_db(db, "publication", "id", pid)


async def list_publications(video_id: str) -> list[dict]:
    """All outcomes for a video, newest first.

    Ordered by ``rowid`` as the tiebreaker, not ``id``: ``created_at`` has
    one-second resolution, so two outcomes recorded in the same second would
    otherwise be ordered by a random UUID — i.e. arbitrarily. ``rowid`` is
    insertion order, which is what "newest first" actually means here.
    """
    db = await get_db()
    cur = await db.execute(
        "SELECT * FROM publication WHERE video_id=? ORDER BY created_at DESC, rowid DESC",
        (video_id,))
    return [dict(r) for r in await cur.fetchall()]


async def list_publications_by_status(status: str) -> list[dict]:
    """Outcomes in a given state — used to find queued retries."""
    db = await get_db()
    cur = await db.execute(
        "SELECT * FROM publication WHERE status=? ORDER BY created_at, rowid", (status,))
    return [dict(r) for r in await cur.fetchall()]


async def list_all_publications(limit: int = 100) -> list[dict]:
    """Every publication outcome, newest first.

    Exists because the other accessors are all scoped to a video or a status, and
    "what has actually been published?" is the first question anyone asks. Same
    ``rowid`` tiebreaker as :func:`list_publications`, for the same reason:
    ``created_at`` has one-second resolution.
    """
    db = await get_db()
    cur = await db.execute(
        "SELECT * FROM publication ORDER BY created_at DESC, rowid DESC LIMIT ?",
        (limit,))
    return [dict(r) for r in await cur.fetchall()]


async def latest_publication(video_id: str, platform: str) -> dict | None:
    """The most recent outcome for one platform, or None."""
    db = await get_db()
    cur = await db.execute(
        """SELECT * FROM publication WHERE video_id=? AND platform=?
           ORDER BY created_at DESC, rowid DESC LIMIT 1""",
        (video_id, (platform or "").strip().lower()))
    row = await cur.fetchone()
    return dict(row) if row else None


async def request_publication_retry(video_id: str, platform: str,
                                    project_id: str = None, reason: str = None) -> dict:
    """Queue a re-attempt for one platform.

    Returns the existing row when a retry is already queued, so pressing the
    button twice does not pile up duplicate work.
    """
    platform = (platform or "").strip().lower()
    existing = await latest_publication(video_id, platform)
    if existing and existing.get("status") == "requested":
        return existing

    return await record_publication(
        video_id=video_id, platform=platform, status="requested",
        metadata={"reason": reason} if reason else None, project_id=project_id,
    )


async def delete_publication(pid: str) -> int:
    return await _delete("publication", "id", pid)
