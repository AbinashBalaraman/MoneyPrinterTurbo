...
from agent.services.stages import stage_for_request_type
...
def _matches_name(entity: dict, names: set) -> bool:
    """Slug OR display-name match — mirrors operations._char_matches."""
    slug = entity.get("slug") or ""
    name = entity.get("name") or ""
    return bool((slug and slug in names) or (name and name in names))


async def _pending_reference_entities(scene: dict, project_id: str) -> list[dict]:
    """Project entities named by the scene that still have no reference image.

    Scene stills are conditioned on the media_id of every character they name,
    so these are the entities that would block a GENERATE_IMAGE.
    """
    ...
    return [
        c for c in await crud.get_project_characters(project_id)
        if _matches_name(c, names) and not c.get("media_id")
    ]


async def _reference_work_in_flight(project_id: str, entities: list[dict]) -> bool:
    """True when a character-reference request for one of these entities is queued or running."""
    if not project_id or not entities:
        return False
    ...
    return any(
        r.get("type") in ("GENERATE_CHARACTER_IMAGE", "REGENERATE_CHARACTER_IMAGE")
        and r.get("status") in ("PENDING", "PROCESSING")
        and r.get("character_id") in ids
        for r in rows
    )
...
async def _prerequisites_met(req: dict, orientation: str) -> bool:
    ...
    # Scene stills need a reference image for every character the scene names.
    # Defer only while that image is genuinely being generated; if nothing is in
    # flight it will never arrive, so let _dispatch surface the error and fail
    # once rather than burning retries on a prerequisite no retry can reach.
    if req_type in ("GENERATE_IMAGE", "REGENERATE_IMAGE"):
        scene = await crud.get_scene(req.get("scene_id"))
        if scene:
            project_id = req.get("project_id")
            waiting = await _pending_reference_entities(scene, project_id)
            if waiting and await _reference_work_in_flight(project_id, waiting):
                logger.info(
                    "IMAGE prereq deferred: scene=%s awaiting reference images for %s",
                    (req.get("scene_id") or "")[:12],
                    ", ".join(c.get("slug") or c.get("name") or "?" for c in waiting),
                )
                return False
...
async def _process_one(req: dict, deferred: dict = None, retry_after: dict = None):
    """Handle one request, tagging every log line it emits with its stage.

    The stage is a contextvar, so it follows the work through every awaited
    call — and into any task spawned from here — without threading a ``stage``
    parameter through the whole call chain.
    """
    with log_stage(stage_for_request_type(req.get("type"))):
        await _process_one_inner(req, deferred, retry_after)


async def _process_one_inner(req: dict, deferred: dict = None, retry_after: dict = None):
...
async def _handle_failure(rid: str, req: dict, result: dict, retry_after: dict = None):
    ...
    # A named entity with no reference image is a pipeline-ordering answer too —
    # no retry conjures the image. _prerequisites_met already defers while the
    # reference request is in flight, so reaching here means nothing is coming.
    # Fail once and name the entities that need GENERATE_CHARACTER_IMAGE.
    if "waiting for reference images" in error_lower:
        detail = str(error_msg).split(":", 1)[-1].strip() or "unknown entity"
        await crud.update_request(
            rid,
            status="FAILED",
            error_message=(
                f"No reference image for: {detail}. "
                f"Queue GENERATE_CHARACTER_IMAGE for those entities, then re-run this request."
            ),
        )
        await _mark_scene_failed(req)
        logger.error(
            "Request %s FAILED (not retryable): no reference image for %s", rid[:8], detail
        )
        return
