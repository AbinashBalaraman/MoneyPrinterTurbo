...
#: Preference order for the timestamp font. A plain sans first, because the
#: overlay is a machine-readable timecode, not typography.
_FONT_PREFERENCE = (
...
def _resolve_font() -> str | None:
    """An explicit font file for ffmpeg's ``drawtext``, or None.

    Why this exists: ``drawtext`` without ``fontfile`` asks fontconfig for a
    default font, and fontconfig has no config on a bare Windows machine — so
    frame extraction died with *"Fontconfig error: Cannot load default config
    file"*. The repo already ships fonts (the assembly engine burns subtitles
    with them), so pointing at one removes the dependency instead of asking every
    machine to grow a fontconfig setup.

    Returns ``None`` when nothing is found, which leaves the previous
    fontconfig-based behaviour in place — a missing font should degrade, not
    crash.
    """
    ...
    font_dir = Path(getattr(config, "FONT_DIR", ""))
    if font_dir.is_dir():
        for name in _FONT_PREFERENCE:
            candidate = font_dir / name
            if candidate.is_file():
                _font_path_cache = _ffmpeg_path(candidate)
    ...
    logger.warning(
        "No font found in %s — frame timestamps will rely on fontconfig, which "
        "fails on a machine without one. Set FONT_DIR in AutoShorts/.env.",
        font_dir,
    )
    _font_path_cache = ""  # sentinel: looked, found nothing
    return None


def _ffmpeg_path(path: Path) -> str:
    """Escape a path for use inside an ffmpeg filter graph.

    Filter options are ``:``-separated, so a Windows drive colon must be escaped
    or ffmpeg reads the drive letter as the start of the next option. Backslashes
    become forward slashes for the same reason.
    """
    return str(path).replace("\\", "/").replace(":", "\\:")


def _timestamp_filter(fps: float) -> str:
    """The scale+timestamp filter chain, with an explicit font when available."""
    parts = [f"fps={fps}", "scale=320:-1"]
    font = _resolve_font()
    fontfile = f"fontfile='{font}':" if font else ""
    parts.append(
        f"drawtext={fontfile}text='%{{pts\\:hms}}':x=5:y=5:fontsize=14:"
...
def _create_contact_sheets(video_path: str, fps: float, out_dir: str) -> tuple[list[Path], int]:
    ...
    for sheet_idx, start in enumerate(range(0, len(frames), per_sheet)):
        ...
        for i, frame_path in enumerate(chunk, start=1):
            # Symlink on POSIX; plain copy on Windows (creating symlinks needs
            # elevation/Developer Mode and dies with WinError 1314 otherwise).
