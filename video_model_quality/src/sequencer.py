...
from .rig import FPS, decode, encode, N_BONES, FEAT_DIM, enforce_ground_contact
from .puppet.choreography import build_motion_from_action
from .parser import parse_prompt
from .catalog import validate_action_params


def blend_feature_clips(feat_a: np.ndarray, feat_b: np.ndarray, overlap_frames: int = 8,
                        handoff: bool = True) -> np.ndarray:
    ...
    ``handoff=False`` keeps clip B's **absolute** root position instead of
    re-anchoring it to clip A's end. Use this for paired/interaction clips that
    must preserve a staged world-space relationship (e.g. two fighters 0.9
    world-units apart); the overlap still ramps the root smoothly from A's end
    to B's absolute start.
    """
    T1 = len(feat_a)
    T2 = len(feat_b)
    
    if overlap_frames <= 0 or T1 < overlap_frames or T2 < overlap_frames:
        # Hard cut without overlap. Still apply root handoff unless the caller
        # asked to preserve clip B's absolute placement.
        feat_b_offset = np.copy(feat_b)
        if handoff:
            offset = feat_a[-1, :2] - feat_b[0, :2]
...
class ActorTrack:
    ...
    def compile(self, overlap_frames: int = 8, return_events: bool = False):
        ...
        if not self.steps:
            ...
            feat = enforce_ground_contact(feat)
            if return_events:
                return feat, []
            return feat
        ...
        if return_events:
            return enforce_ground_contact(current), shifted
        return enforce_ground_contact(current)
