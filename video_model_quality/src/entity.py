...
class StickmanEntity:
    ...
    # -- contact sensors ----------------------------------------------
    def foot_contacts(self, ground_y=None, tol=0.02):
        """Binary foot-ground contact from ankle height vs ground plane.

        Defaults to ``ENTITY_GROUND_Y`` — entity screen space (+y DOWN). This is
        deliberately NOT ``rig.GROUND_Y``, which is world space (+y UP, negative).
        """
        if ground_y is None:
            ground_y = ENTITY_GROUND_Y
        p = self.fk()
        return {f: bool(p[f][1] >= ground_y - tol)
                for f in ("ankle_l", "ankle_r")}


# Ground plane in ENTITY screen space (+y DOWN) — derived from the rest pose so
# it tracks any future leg-length change. NOT the same value as rig.GROUND_Y.
def _rest_ground_screen_y() -> float:
    _ent = StickmanEntity()
    _p = _ent.fk()
    return float(max(_p[f][1] for f in ("ankle_l", "ankle_r")))


ENTITY_GROUND_Y = _rest_ground_screen_y()
