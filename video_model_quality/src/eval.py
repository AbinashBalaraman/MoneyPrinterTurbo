...
from .catalog import get_action_catalog, ACTION_METADATA, ACTIONS_1P, ACTIONS_2P
...
BENCHMARK_PROMPTS = [
    ...
    "stickman guards against danger",
...
def run_benchmark_suite() -> Dict[str, Any]:
    ...
    # Test Determinism using the real engine path (same seed -> bitwise identical)
    from .puppet.choreography import build_motion_from_action
    clip_a = build_motion_from_action("walk", duration_s=2.0, seed=42)
    clip_b = build_motion_from_action("walk", duration_s=2.0, seed=42)
    determinism_ok = np.array_equal(clip_a, clip_b)

    # Test Diversity (different seeds must produce distinct output)
    clip_c = build_motion_from_action("walk", duration_s=2.0, seed=43)
