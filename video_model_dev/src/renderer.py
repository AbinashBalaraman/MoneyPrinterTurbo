...
from .rig import decode, forward_kinematics, BONES, FPS, CLIP_LEN_5S
...
WORLD_GROUND_Y = -0.42  # Rest foot Y in rig.py
...
def render_to_video(feat_or_joints, output_path, fps=FPS, canvas_size=512,
    ...
    if vfx_events:
        impacts = impulses_from_impact_events(vfx_events)
        if impacts:
            cam_track = np.asarray(apply_camera_impulses(cam_track, impacts),
                                   dtype=np.float32)
    ...
    try:
        for t in range(T):
            ...
            img = renderer.render_frame(joints[t], camera_x=cam_track[t], props=props,
                                        vfx=frame_vfx)
