...
from src.rig import BONES, N_BONES, FPS, decode, encode, forward_kinematics
from src.puppet.combat_ik import (capsule_matrix, capsule_contacts,
                                  RIG_CAPSULE_R, STRIKERS)

GROUND_Y = -0.42  # Calibrated world ground plane
...
class VisualInspector:
    ...
    def audit_motion(self, feat: np.ndarray, name: str = "motion_clip",
        ...
        vel = np.diff(angles, axis=0) * self.fps  # (T-1, 14)
        ...
        # Feet: toe_l (11), ankle_l (10), toe_r (14), ankle_r (13)
        for foot_j in (10, 11, 13, 14):
        ...
        ang_vel = np.linalg.norm(np.diff(angles, axis=0), axis=-1)  # (T-1,)
